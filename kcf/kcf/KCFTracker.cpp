...
namespace kcf {
	...
	kcf_tracker::kcf_tracker(float _padding, float _output_sigma_factor, float _kernel_sigma, 
	...
	{
		if (_padding < 0 && _padding > 2) padding = 1.5f;
		else padding = _padding;

		if (_output_sigma_factor < 0.01 && _output_sigma_factor > 0.1) output_sigma_factor = 0.1f;
		else output_sigma_factor = _output_sigma_factor;

		if (_kernel_sigma < 0.01 && _kernel_sigma > 1.0f) kernel_sigma = 0.5f; // kernel_sigma = 0.2f;
		else kernel_sigma = _kernel_sigma;
		...
		if (_learning_rate < 0 && _learning_rate > 1) learning_rate = 0.02f; // learning_rate = 0.075f;
		...
		//dout = 1; cell_size = 1;   kernel_sigma = 0.2f; learning_rate = 0.075f; // gray
	    dout = 31;  cell_size = 4;  // kernel_sigma = 0.5f; learning_rate = 0.02f;  // hog - all-zeros channel features are not considered
		dscale = 31; 
		
		xt = (float**)fftwf_malloc(sizeof(float*) * dout);
		xtf = (fftwf_complex**)fftwf_malloc(sizeof(fftwf_complex*) * dout);
		model_xtf = (fftwf_complex**)fftwf_malloc(sizeof(fftwf_complex*) * dout);
	
		xxf = (fftwf_complex**)fftwf_malloc(sizeof(fftwf_complex*) * dout);
		xy = (float**)fftwf_malloc(sizeof(float*) * dout);
	...
	kcf_tracker::~kcf_tracker()
	{
		...
		for (int i = 0; i < dout; i++) fftwf_free(model_xtf[i]);
		fftwf_free(model_xtf);

		for (int i = 0; i < dout; i++) fftwf_free(xy[i]);
		fftwf_free(xy);
		...
		fftwf_free(alphaf);
		fftwf_free(model_alphaf);
		
		for (int i = 0; i < dout; i++) fftwf_free(xxf[i]);
		fftwf_free(xxf);


		fftwf_free(sxy);

		fftwf_destroy_plan(pkf);

		fftwf_cleanup();
	}

	void kcf_tracker::make_translation_hann_window(int n, float* window)
	{
		for (int i = 0; i < n; i++) {
	...
	void kcf_tracker::make_scale_hann_window(int n, float *window)
	{
		if (n > nScales)
			for (int i = 1; i < n; i++)
	...
	void kcf_tracker::make_scale_cosine_mask()
	{
		if (nScales % 2 == 0)
	...
	void kcf_tracker::make_translation_cosine_mask()
	{
	...
	bool kcf_tracker::initializeTargetModel(int c_x, int c_y, int t_w, int t_h, int _imw, int _imh, unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB)
	{
		...
		alphaf = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * prodszhalf);
		memset(alphaf, 0, sizeof(fftwf_complex) * prodszhalf);
		model_alphaf = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * prodszhalf);
		memset(model_alphaf, 0, sizeof(fftwf_complex) * prodszhalf);
		...
		for (int rs = dy; rs < csz[0] + dy; rs++)
			...
			{     
				if (cs < 0) { // левее
					if (rs < 0)
						in = (csz[1] + cs) + csz[1] * (csz[0] + rs);  // левее
					else
						in = (csz[1] + cs) + csz[1] * (rs - dy + dy); // правее или равно пику
				}
				else // правее или равно пику
					if (rs < 0)
		...
		kf = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * prodszhalf);
		for (int i = 0; i < dout; i++)
			xtf[i] = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * prodszhalf);

		for (int i = 0; i < dout; i++)
			model_xtf[i] = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * prodszhalf);


		for (int i = 0; i < dout; i++)
			xxf[i] = (fftwf_complex*)fftwf_malloc(sizeof(fftwf_complex) * prodszhalf);

		for (int i = 0; i < dout; i++)
			xy[i] = (float*)fftwf_malloc(sizeof(float) * csz[1] * csz[0]);

		sxy = (float*)fftwf_malloc(sizeof(float) * prodsz);

		pkf = fftwf_plan_dft_r2c_2d(csz[0], csz[1], sxy, kf, FFTW_MEASURE);
		...
		pxxf = (fftwf_plan*)fftwf_malloc(sizeof(fftwf_plan) * dout);
		for (int j = 0; j < dout; j++)
			pxxf[j] = fftwf_plan_dft_c2r_2d(csz[0], csz[1], xxf[j], xy[j], FFTW_MEASURE); // FFTW_ESTIMATE);
	...
	void kcf_tracker::get_translation_feature_map(float *In, float **Out, int h, int w, int din)
	{
	...
	void kcf_tracker::get_scale_feature_map(float *In, int h, int w, int din, float **scale_sample, int s)
	{
		// compute fhog features in col-major order
		int n = h*w;
		//float *M = new float[n]; 
	...
	void kcf_tracker::extract_image(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB,
	...
	void kcf_tracker::get_translation_sample(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB, float **sample) // data in row-major order
	{
	...
	void kcf_tracker::get_scale_sample(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB, float **scale_sample)
	{
	...
	void kcf_tracker::extract_training_sample_info(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB, bool first)
	{
		...
		for (int i = 0; i < dout; i++) {
			for (int j = 0; j < prodszhalf; j++) {
				if (j % (csz[1] / 2 + 1) == 0) 
					xx += (xtf[i][j][0] * xtf[i][j][0] + xtf[i][j][1] * xtf[i][j][1]);
				else
					xx += 2 * (xtf[i][j][0] * xtf[i][j][0] + xtf[i][j][1] * xtf[i][j][1]);		
			}
		}
		xx /= (prodsz);
		...
		for (int i = 0; i < dout; i++)
			for (int j = 0; j < prodszhalf; j++) {
				xxf[i][j][0] = xtf[i][j][0] * xtf[i][j][0] + xtf[i][j][1] * xtf[i][j][1];
				xxf[i][j][1] = -xtf[i][j][0] * xtf[i][j][1] + xtf[i][j][1] * xtf[i][j][0];
			}

		for (int i = 0; i < dout; i++) {
			fftwf_execute(pxxf[i]);
		}

		for (int i = 0; i < dout; i++)
			for (int j = 0; j < csz[0] * csz[1]; j++)
				xy[i][j] /= csz[0]*csz[1];

		memset(sxy, 0, sizeof(float) * csz[0] * csz[1]);
		for (int i = 0; i < dout; i++)
			for (int j = 0; j < csz[0] * csz[1]; j++)
				sxy[j] += xy[i][j];

		//calculate gaussian response for all positions, then go back to the Fourier domain
		float kernel_sigma2inv = -1.0f / (kernel_sigma * kernel_sigma);
		for (int j = 0; j < csz[0] * csz[1]; j++)
			sxy[j] = std::exp(kernel_sigma2inv * std::max(0.0f, (2*xx - 2*sxy[j]) / float(csz[0]*csz[1]*dout))); // умножение рассчитывать один раз, всюду поправить
	
		memset(kf, 0, sizeof(fftwf_complex) * prodszhalf);
		...
		if (first == true) {
			...
			for (int j = 0; j < prodszhalf; j++) {
				model_alphaf[j][0] = alphaf[j][0];
				model_alphaf[j][1] = alphaf[j][1];
			}
			...
			for (int j = 0; j < prodszhalf; j++) {
				model_alphaf[j][0] = (1 - learning_rate) * model_alphaf[j][0] + learning_rate * alphaf[j][0];
				model_alphaf[j][1] = (1 - learning_rate) * model_alphaf[j][1] + learning_rate * alphaf[j][1];
			}
			for (int i = 0; i < dout; i++)
				for (int j = 0; j < prodszhalf; j++) {
					model_xtf[i][j][0] = (1 - learning_rate) * model_xtf[i][j][0] + learning_rate * xtf[i][j][0];
					model_xtf[i][j][1] = (1 - learning_rate) * model_xtf[i][j][1] + learning_rate * xtf[i][j][1];
				}
		}
	...
	void kcf_tracker::extract_translation_test_sample(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB)
	{
	...
	void kcf_tracker::extract_scale_test_sample(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB)
	{
	...
	bool kcf_tracker::findNextLocation(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB)
	{
		//  ***************** TRANSLATION PART *************************//
		...
		// linear cross - correlation term in Fourier domain	
		float xx = 0;
		for (int i = 0; i < dout; i++) {
			for (int j = 0; j < prodszhalf; j++)
				if (j % (csz[1] / 2 + 1) == 0)
		...
		for (int i = 0; i < dout; i++) {
			for (int j = 0; j < prodszhalf; j++)
				if (j % (csz[1] / 2 + 1) == 0) 
					yy += (model_xtf[i][j][0] * model_xtf[i][j][0] + model_xtf[i][j][1] * model_xtf[i][j][1]);
				else
					yy += 2 * (model_xtf[i][j][0] * model_xtf[i][j][0] + model_xtf[i][j][1] * model_xtf[i][j][1]);
		}
		yy /= prodsz;
		...
		for (int i = 0; i < dout; i++)
			for (int j = 0; j < prodszhalf; j++) {
				xxf[i][j][0] = xtf[i][j][0] * model_xtf[i][j][0] + xtf[i][j][1] * model_xtf[i][j][1];
				xxf[i][j][1] = -xtf[i][j][0] * model_xtf[i][j][1] + xtf[i][j][1] * model_xtf[i][j][0];
			}
		...
		float kernel_sigma2inv = -1.0f / (kernel_sigma * kernel_sigma);
		for (int j = 0; j < csz[0] * csz[1]; j++)
			sxy[j] = std::exp(kernel_sigma2inv * std::max(0.0f, (xx + yy - 2 * sxy[j]) / float(csz[0] * csz[1] * dout))); // умножение рассчитывать один раз, всюду поправить
		...
		//model_alphaf .* kf
		for (int j = 0; j < prodszhalf; j++) {
			rt[j][0] = model_alphaf[j][0] * kf[j][0] - model_alphaf[j][1] * kf[j][1];
			rt[j][1] = model_alphaf[j][0] * kf[j][1] + model_alphaf[j][1] * kf[j][0];
		}	
	...
	bool kcf_tracker::getNewLocationCoordinates(int &x, int &y, int &w, int &h, float &scr)
	{
		if (true) {
			x = ix;
			y = iy;
