...
namespace mosse {
	...
	void mosse_tracker::get_translation_sample(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB, float **sample) // data in row-major order
	{
	...
	void mosse_tracker::extract_training_sample_info(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB, bool first)
	{
	    ...
	    // kf = sum(xtf.*conj(xtf), 3) / numel(xtf) <-- idea 
	...
	void mosse_tracker::extract_translation_test_sample(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB)
	{
	...
	bool mosse_tracker::findNextLocation(unsigned char* dataYorR, unsigned char* dataG, unsigned char* dataB)
	{
		...
		// linear cross - correlation term in Fourier domain	
		// kzf = sum(xtf.*conj(model_xtf), 3) / numel(xtf) <-- idea 	
		memset(kf, 0, sizeof(fftwf_complex) * prodszhalf);
		for (int i = 0; i < dout; i++)
			for (int j = 0; j < prodszhalf; j++) {	
				kf[j][0] += xtf[i][j][0] * model_xtf[i][j][0] + xtf[i][j][1] * model_xtf[i][j][1];
				kf[j][1] += -xtf[i][j][0] * model_xtf[i][j][1] + xtf[i][j][1] * model_xtf[i][j][0];
			}

		for (int j = 0; j < prodszhalf; j++) {
			kf[j][0] /= (csz[0] * csz[1] * dout);
			kf[j][1] /= (csz[0] * csz[1] * dout);
		}
